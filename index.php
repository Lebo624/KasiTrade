...
function requireLogin() { if (!isLoggedIn()) { http_response_code(401); echo json_encode(['error' => 'Login required']); exit; } }
function requireAdmin() { if (!isAdmin()) { http_response_code(403); echo json_encode(['error' => 'Admin access only']); exit; } }
...
if (isset($_GET['action'])) {
    ...
    if ($action === 'editProfile') {
        ...
        if ($newPassword) $_SESSION['user']['password_hash'] = password_hash($newPassword, PASSWORD_DEFAULT);
    ...
    if ($action === 'placeOrder') {
        ...
        echo json_encode(['success' => true, 'order_id' => $pdo->lastInsertId()]); exit;
    }
    ...
    if ($action === 'getFavorites') {
        ...
        $stmt->execute([getCurrentUser()['id']]); echo json_encode($stmt->fetchAll()); exit;
    }
    ...
    if ($action === 'adminStats') {
        ...
        echo json_encode(['products' => (int)$products, 'users' => (int)$users, 'orders' => (int)$orders]); exit;
    }
    ...
    if ($action === 'adminGetUsers') {
        requireAdmin();
        $stmt = $pdo->query('SELECT id, email, name, surname, role, profile_image, is_verified, created_at FROM users ORDER BY created_at DESC');
        echo json_encode($stmt->fetchAll()); exit;
    }

    if ($action === 'adminGetAllOrders') {
        requireAdmin();
        $stmt = $pdo->query('SELECT o.*, p.name AS product_name, buyer.email AS buyer_email, seller.email AS seller_email FROM orders o JOIN products p ON o.product_id = p.id JOIN users buyer ON o.buyer_id = buyer.id JOIN users seller ON o.seller_id = seller.id ORDER BY o.created_at DESC');
        echo json_encode($stmt->fetchAll()); exit;
    }
...
<body>
  ...
  <main class="container">
    <section id="home" class="page active">
      <div class="hero"><h1>Trade with your Kasi</h1><p>Buy and sell directly within your community. Simple, fast, and built for side‑hustlers.</p><a href="#" class="btn" onclick="showPage('products')">Browse Products</a><a href="#" class="btn btn-outline" style="margin-left:0.5rem;" onclick="showPage('sell')">Start Selling</a></div>
      <h2 style="margin-bottom:1rem;">Featured Products</h2><div class="product-grid" id="featuredGrid"></div>
      <h2 style="margin:2rem 0 1rem;">Trending</h2><div class="product-grid" id="homeProductGrid"></div>
      <div style="text-align:center;margin:2rem 0;"><a href="#" class="btn btn-outline" onclick="showPage('products')">View All Listings</a></div>
    </section>

    <section id="products" class="page">
      <h2>All Products</h2>
      <div class="search-bar"><input type="text" id="searchInput" placeholder="Search products..."><select id="categoryFilter"><option value="">All Categories</option><option value="Fashion">Fashion</option><option value="Electronics">Electronics</option><option value="Furniture">Furniture</option><option value="Food">Food</option></select><input type="number" id="priceMin" placeholder="Min R" style="flex:0.5;"><input type="number" id="priceMax" placeholder="Max R" style="flex:0.5;"></div>
      <div class="product-grid" id="allProductsGrid"></div>
    </section>

    <section id="productDetails" class="page"><div id="productDetailContent" class="card" style="max-width:600px;"></div></section>

    <section id="sell" class="page"><h2>Sell a Product</h2><div class="card" style="max-width:500px;"><form id="sellForm"><div class="form-group"><label>Product Name</label><input type="text" class="form-control" id="sellName" required></div><div class="form-group"><label>Description</label><textarea class="form-control" id="sellDesc" rows="3" required></textarea></div><div class="form-group"><label>Price (R)</label><input type="number" class="form-control" id="sellPrice" min="1" required></div><div class="form-group"><label>Category</label><select class="form-control" id="sellCategory" required><option value="">Select category</option><option value="Fashion">Fashion</option><option value="Electronics">Electronics</option><option value="Furniture">Furniture</option><option value="Food">Food</option></select></div><div class="form-group"><label>Product Image</label><div class="file-upload-wrapper"><div class="file-upload-trigger" id="sellUploadTrigger">Choose image or paste URL below</div><input type="file" id="sellFileInput" accept="image/*"></div><input type="text" class="form-control" id="sellImageUrl" placeholder="Or paste image URL" style="margin-top:0.5rem;"><img id="sellImagePreview" class="image-preview" alt="Preview"></div><button type="submit" class="btn">List Product</button></form></div></section>
  ...
  <div class="card" style="max-width:400px;margin-top:1.5rem;">
    ...
    <section id="profile" class="page">
      ...
      <div class="card" style="max-width:500px; text-align:center;">
        <img id="profileImage" class="profile-pic" src="" alt="Profile" style="display:none; margin-bottom:1rem;">
        <h3 id="profileFullName"></h3>
        <p id="profileEmailDisplay"></p>
        <p><span id="profileVerifiedBadge" class="profile-verified" style="display:none;">Verified</span> <span id="profileRoleBadge"></span></p>
    ...
    <section id="editProfile" class="page">
      ...
      <div class="card" style="max-width:400px;">
        <form id="editProfileForm" enctype="multipart/form-data">
          <div class="form-group"><label>First Name</label><input type="text" class="form-control" id="editProfileName" required></div>
          <div class="form-group"><label>Surname</label><input type="text" class="form-control" id="editProfileSurname" required></div>
          <div class="form-group">
            <label>Profile Picture</label>
            <div class="file-upload-wrapper">
              <div class="file-upload-trigger" id="editProfilePicTrigger">Choose image</div>
              <input type="file" id="editProfilePicInput" accept="image/*" style="display:none;">
            </div>
            <img id="editProfilePicPreview" class="image-preview" style="max-width:100px; max-height:100px; border-radius:50%; margin-top:0.5rem;">
          </div>
    ...
    <section id="orders" class="page"><h2>My Requests / Orders</h2><div id="ordersList"></div></section>

    <section id="messages" class="page">
      <h2>Messages for Order <span id="msgOrderId"></span></h2>
      <div id="messagesContainer" style="max-height:400px; overflow-y:auto; margin-bottom:1rem; padding:0 1rem;"></div>
      <div class="card" style="max-width:500px; margin:0 auto;">
        <form id="messageForm">
          <input type="hidden" id="msgOrderIdInput">
          <input type="hidden" id="msgReceiverId">
          <div class="form-group"><textarea class="form-control" id="msgText" rows="2" placeholder="Type a message..." required></textarea></div>
  ...
  <script>
    ...
    async function api(action, data = {}, method = 'POST') {
        ...
        if (method === 'GET') { const params = new URLSearchParams(data).toString(); if (params) url += '&' + params; }
        else { options.headers['Content-Type'] = 'application/json'; if (data && Object.keys(data).length) options.body = JSON.stringify(data); }
        const res = await fetch(url, options); const json = await res.json();
    ...
    function updateAuthUI() {
        ...
        if (currentUser) {
            fetchUnreadCount();
            authLinks.innerHTML = `<a onclick="showPage('sell')">Sell</a><a onclick="showPage('myListings')">My Listings</a><a onclick="showPage('mySales')">Sales</a><a onclick="showPage('orders')">Orders</a><a onclick="showPage('favorites')">Favorites</a><a onclick="showPage('notifications')" style="position:relative;">Notifications <span class="badge-pill" id="notifCount" style="display:none;"></span></a><a onclick="showPage('profile')">${currentUser.name}</a>${currentUser.role === 'admin' ? '<a onclick="showPage(\'admin\')" style="color:#2a9d8f;">Admin</a>' : ''}<a onclick="handleLogout()" style="color:#e76f51;">Logout</a>`;
        } else {
            authLinks.innerHTML = `<a onclick="showPage('login')">Login</a><a onclick="showPage('register')">Register</a>`;
        }
    }

    async function fetchUnreadCount() {
        try { const data = await api('unreadNotifications', {}, 'GET'); const badge = document.getElementById('notifCount'); if (badge) { if (data.count > 0) { badge.textContent = data.count; badge.style.display = 'inline'; } else { badge.style.display = 'none'; } } } catch (e) {}
    }
    ...
    function showPage(pageId, param = null) {
        ...
        if (pageId === 'messages') {
            document.getElementById('msgOrderIdInput').value = param;
            document.getElementById('msgReceiverId').value = currentMessageReceiverId;
            renderMessages(param);
        }
        document.querySelectorAll('.nav-links a.active').forEach(l => l.classList.remove('active'));
        document.querySelectorAll('.nav-links a').forEach(link => { if (link.getAttribute('onclick')?.includes(`'${pageId}'`)) link.classList.add('active'); });
    ...
    function createProductCard(product) {
        ...
        return `<div class="card product-card" onclick="showPage('productDetails', ${product.id})">${favHTML}<img src="${product.image_path || 'https://placehold.co/400x250/2a9d8f/ffffff?text=No+Image'}" alt="${product.name}"><span class="category-tag">${product.category}</span>${featuredTag}<h3>${product.name}</h3><p class="price">R${parseFloat(product.price).toFixed(2)}</p><p class="seller">By: ${product.seller_name}</p></div>`;
    }

    async function fetchProducts(filters = {}) { return await api('getProducts', filters, 'GET'); }
    async function renderFeatured() { const products = await fetchProducts(); const featured = products.filter(p => p.is_featured).slice(0, 4); document.getElementById('featuredGrid').innerHTML = featured.length ? featured.map(createProductCard).join('') : '<p>No featured products yet.</p>'; }
    async function renderHomeProducts() { const products = await fetchProducts(); const nonFeatured = products.filter(p => !p.is_featured).slice(0, 4); document.getElementById('homeProductGrid').innerHTML = nonFeatured.map(createProductCard).join(''); }
    async function renderAllProducts(filtered = null) { const filters = filtered || {}; const products = await fetchProducts(filters); document.getElementById('allProductsGrid').innerHTML = products.length ? products.map(createProductCard).join('') : '<p>No products found.</p>'; }

    async function renderProductDetails(id) {
        const product = await api('getProduct', { id }, 'GET'); const content = document.getElementById('productDetailContent');
        ...
        const isFav = currentUser ? product.is_favorite : false;
        content.innerHTML = `<img src="${product.image_path}" style="width:100%; height:250px; object-fit:cover; border-radius:12px; margin-bottom:1rem;"><span class="category-tag">${product.category}</span>${product.is_featured ? '<span class="category-tag featured-badge">Featured</span>' : ''}<h2>${product.name}</h2><p style="font-size:1.8rem; color:var(--accent);">R${parseFloat(product.price).toFixed(2)}</p><p><strong>Seller:</strong> ${product.seller_name}</p><p style="margin:1rem 0;">${product.description}</p><button class="btn" onclick="handleRequestProduct(${product.id})">Request to Buy</button><button class="btn btn-outline" style="margin-left:0.5rem;" onclick="handleToggleFavorite(${product.id})">${isFav ? '❤️ Remove from Favorites' : '🤍 Add to Favorites'}</button><button class="btn btn-outline" style="margin-left:0.5rem;" onclick="showPage('products')">Back</button>${canEdit ? `<button class="btn btn-outline" style="margin-left:0.5rem;" onclick="showPage('editProduct', ${product.id})">Edit</button><button class="btn btn-danger" style="margin-left:0.5rem;" onclick="handleDeleteProduct(${product.id})">Delete</button>` : ''}`;
    }

    async function handleToggleFavorite(productId) { if (!currentUser) { alert('Please login.'); showPage('login'); return; } await api('toggleFavorite', { product_id: productId }, 'POST'); const cp = document.querySelector('.page.active').id; if (cp === 'productDetails') renderProductDetails(productId); else if (cp === 'home') { renderFeatured(); renderHomeProducts(); } else if (cp === 'products') filterProducts(); else if (cp === 'favorites') renderFavorites(); }
    async function renderFavorites() { if (!currentUser) { document.getElementById('favoritesGrid').innerHTML = '<p>Please login.</p>'; return; } const favs = await api('getFavorites', {}, 'GET'); document.getElementById('favoritesGrid').innerHTML = favs.length ? favs.map(createProductCard).join('') : '<p>No favorites yet.</p>'; }

    async function renderMyListings() { if (!currentUser) { document.getElementById('myListingsContainer').innerHTML = '<p>Please login.</p>'; return; } const all = await fetchProducts(); const mine = all.filter(p => p.seller_id == currentUser.id); document.getElementById('myListingsContainer').innerHTML = mine.length ? mine.map(p => `<div class="card list-item" style="margin-bottom:0.5rem;"><div><strong>${p.name}</strong> – R${p.price} <span class="category-tag">${p.category}</span></div><div class="actions"><a href="#" class="btn btn-sm" onclick="event.stopPropagation(); showPage('editProduct', ${p.id})">Edit</a><a href="#" class="btn btn-sm btn-danger" onclick="event.stopPropagation(); handleDeleteProduct(${p.id})">Delete</a></div></div>`).join('') : '<p>No listings yet.</p>'; }
    async function prefillEditForm(id) { const p = await api('getProduct', { id }, 'GET'); if (!p || p.error) { alert('Not found.'); showPage('myListings'); return; } document.getElementById('editId').value = p.id; document.getElementById('editName').value = p.name; document.getElementById('editDesc').value = p.description; document.getElementById('editPrice').value = p.price; document.getElementById('editCategory').value = p.category; document.getElementById('editImageUrl').value = p.image_path || ''; const preview = document.getElementById('editImagePreview'); if (p.image_path) { preview.src = p.image_path; preview.style.display = 'block'; } else { preview.style.display = 'none'; } }
    async function handleDeleteProduct(id) { if (!confirm('Delete?')) return; await api('deleteProduct', { id }, 'POST'); alert('Deleted!'); showPage('myListings'); }

    document.getElementById('sellForm').addEventListener('submit', async function(e) { e.preventDefault(); if (!currentUser) { alert('Login to sell.'); showPage('login'); return; } const data = { name: document.getElementById('sellName').value, desc: document.getElementById('sellDesc').value, price: parseFloat(document.getElementById('sellPrice').value), category: document.getElementById('sellCategory').value, image: document.getElementById('sellImageUrl').value || 'https://placehold.co/400x250/2a9d8f/ffffff?text=' + encodeURIComponent(document.getElementById('sellName').value) }; await api('addProduct', data, 'POST'); alert('Product listed!'); showPage('myListings'); });
    document.getElementById('editForm').addEventListener('submit', async function(e) { e.preventDefault(); const id = parseInt(document.getElementById('editId').value); const data = { id, name: document.getElementById('editName').value, desc: document.getElementById('editDesc').value, price: parseFloat(document.getElementById('editPrice').value), category: document.getElementById('editCategory').value, image: document.getElementById('editImageUrl').value || 'https://placehold.co/400x250/2a9d8f/ffffff?text=' + encodeURIComponent(document.getElementById('editName').value) }; await api('updateProduct', data, 'POST'); alert('Updated!'); showPage('myListings'); });

    async function handleRequestProduct(productId) { if (!currentUser) { alert('Login to request.'); showPage('login'); return; } await api('placeOrder', { product_id: productId }, 'POST'); alert('Request sent!'); showPage('orders'); }
    async function renderMyOrders() { if (!currentUser) { document.getElementById('ordersList').innerHTML = '<p>Please login.</p>'; return; } const orders = await api('getOrders', { type: 'buyer' }, 'GET'); document.getElementById('ordersList').innerHTML = orders.length ? orders.map(o => `<div class="card" style="margin-bottom:0.5rem;"><strong>${o.product_name}</strong> – R${o.product_price}<br><small>${o.created_at} | <span class="badge ${o.status.toLowerCase()}">${o.status}</span></small><br><button class="btn btn-sm" onclick="event.stopPropagation(); openMessages(${o.id}, ${o.seller_id})">Message Seller</button></div>`).join('') : '<p>No requests yet.</p>'; }
    async function renderMySales() { if (!currentUser) { document.getElementById('mySalesContainer').innerHTML = '<p>Please login.</p>'; return; } const orders = await api('getOrders', { type: 'seller' }, 'GET'); document.getElementById('mySalesContainer').innerHTML = orders.length ? orders.map(o => `<div class="card" style="margin-bottom:0.5rem;"><strong>${o.product_name}</strong> – R${o.product_price} (Buyer: ${o.buyer_email})<br><small>${o.created_at} | <span class="badge ${o.status.toLowerCase()}">${o.status}</span></small><br><button class="btn btn-sm" onclick="event.stopPropagation(); openMessages(${o.id}, ${o.buyer_id})">Message Buyer</button></div>`).join('') : '<p>No sales yet.</p>'; }

    function openMessages(orderId, receiverId) {
        currentMessageReceiverId = receiverId;
        showPage('messages', orderId);
    }

    async function renderMessages(orderId) {
        const msgs = await api('getMessages', { order_id: orderId }, 'GET');
        const container = document.getElementById('messagesContainer');
        container.innerHTML = msgs.map(m => `
            <div class="message-bubble ${m.sender_id == currentUser.id ? 'mine' : ''}">
                <p>${m.message}</p>
                <p class="message-meta">${m.sender_name} - ${m.created_at}</p>
            </div>
        `).join('');
    ...
    document.getElementById('messageForm').addEventListener('submit', async function(e) {
        e.preventDefault();
        const orderId = document.getElementById('msgOrderIdInput').value;
        const receiverId = document.getElementById('msgReceiverId').value;
        const text = document.getElementById('msgText').value.trim();
        if (!text) return;
        await api('sendMessage', { order_id: orderId, receiver_id: receiverId, message: text }, 'POST');
        document.getElementById('msgText').value = '';
        renderMessages(orderId);
    });

    async function renderNotifications() { if (!currentUser) { document.getElementById('notificationsContainer').innerHTML = '<p>Please login.</p>'; return; } const notifs = await api('getNotifications', {}, 'GET'); document.getElementById('notificationsContainer').innerHTML = notifs.length ? notifs.map(n => `<div class="notif-item ${n.is_read ? '' : 'unread'}"><p>${n.message}</p><small>${n.created_at}</small></div>`).join('') : '<p>No notifications.</p>'; fetchUnreadCount(); }
    ...
    async function renderProfile() {
        ...
        document.getElementById('profileRoleBadge').textContent = 'Role: ' + (currentUser.role || 'user');
        const pic = document.getElementById('profileImage');
        if (currentUser.profile_image) { pic.src = currentUser.profile_image; pic.style.display = 'block'; } else { pic.style.display = 'none'; }
        const verified = document.getElementById('profileVerifiedBadge');
        if (currentUser.is_verified) { verified.style.display = 'inline-block'; } else { verified.style.display = 'none'; }
    }

    function fillEditProfile() {
        if (!currentUser) { showPage('login'); return; }
        document.getElementById('editProfileName').value = currentUser.name || '';
        document.getElementById('editProfileSurname').value = currentUser.surname || '';
        ...
        if (currentUser.profile_image) { preview.src = currentUser.profile_image; preview.style.display = 'block'; } else { preview.style.display = 'none'; }
    }
    ...
    document.getElementById('editProfileForm').addEventListener('submit', async function(e) {
        ...
        const data = {
            ...
            surname: document.getElementById('editProfileSurname').value,
            newPassword: document.getElementById('editProfileNewPassword').value
        };
        try {
            await api('editProfile', data, 'POST');
            const fileInput = document.getElementById('editProfilePicInput');
            if (fileInput.files.length > 0) {
                const formData = new FormData();
                formData.append('image', fileInput.files[0]);
                const uploadRes = await fetch(window.location.pathname + '?action=uploadProfilePicture', { method: 'POST', body: formData });
                const uploadJson = await uploadRes.json();
                if (uploadJson.success) currentUser.profile_image = uploadJson.image;
            }
    ...
    document.getElementById('editProfilePicTrigger').addEventListener('click', () => document.getElementById('editProfilePicInput').click());
    document.getElementById('editProfilePicInput').addEventListener('change', function() {
        const file = this.files[0];
        if (file) {
            if (file.size > 2 * 1024 * 1024) { alert('Image too large (max 2MB).'); this.value = ''; return; }
            const reader = new FileReader();
            reader.onload = (e) => { document.getElementById('editProfilePicPreview').src = e.target.result; document.getElementById('editProfilePicPreview').style.display = 'block'; };
...
document.getElementById('regProfilePicInput').addEventListener('change', function() {
    ...
    async function filterProducts() { const filters = { search: document.getElementById('searchInput').value, category: document.getElementById('categoryFilter').value, minPrice: document.getElementById('priceMin').value || 0, maxPrice: document.getElementById('priceMax').value || 999999 }; renderAllProducts(filters); }

    function setupImageUpload(fileInputId, triggerId, previewId, urlInputId) { const fileInput = document.getElementById(fileInputId); const trigger = document.getElementById(triggerId); const preview = document.getElementById(previewId); const urlInput = document.getElementById(urlInputId); trigger.addEventListener('click', () => fileInput.click()); fileInput.addEventListener('change', () => { const file = fileInput.files[0]; if (file) { if (file.size > 2 * 1024 * 1024) { alert('Image too large (max 2MB).'); fileInput.value = ''; return; } const reader = new FileReader(); reader.onload = (e) => { urlInput.value = e.target.result; preview.src = e.target.result; preview.style.display = 'block'; }; reader.readAsDataURL(file); } }); urlInput.addEventListener('input', () => { const val = urlInput.value.trim(); if (val) { preview.src = val; preview.style.display = 'block'; } else { preview.style.display = 'none'; } }); }
